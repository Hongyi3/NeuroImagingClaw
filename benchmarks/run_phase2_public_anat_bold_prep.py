#!/usr/bin/env python3
"""Run the pinned public anat/BOLD preprocessing benchmark for Phase 2 evidence."""
...
from clawneuro.reporting import harvest_fmriprep_outputs  # noqa: E402
from clawneuro.skills.anat_bold_prep import AnatBoldPrepConfig, run_anat_bold_prep  # noqa: E402
...
def main() -> int:
    ...
    harvest = harvest_fmriprep_outputs(output_root / "derivatives")
    preserved_outputs = copy_preserved_derivative_files(
        output_root / "derivatives",
        artifact_root,
        list(harvest.report_paths)
        + list(harvest.confounds_paths)
        + list(harvest.boilerplate_paths)
        + ([harvest.dataset_description_path] if harvest.dataset_description_path else []),
    )

    run_manifest = json.loads((artifact_root / "manifests" / "run-manifest.json").read_text(encoding="utf-8"))
    summary = json.loads((artifact_root / "manifests" / "anat-bold-prep-summary.json").read_text(encoding="utf-8"))

    metadata = benchmark_header("phase2_ds003020_anat_bold_prep")
    metadata.update(
        {
            ...
            "artifacts": {
                ...
                "summary_manifest": "manifests/anat-bold-prep-summary.json",
                "report": "report/anat-bold-prep-report.md",
